...
Automated packaging of [Claude Code](https://github.com/anthropics/claude-code) into self-contained, portable bundles. Bundles include a JavaScript runtime (Node.js or Deno) so no global install is needed. Builds run daily via GitHub Actions, detecting upstream version changes and producing 12 artifacts per release cycle.
...
│ versions.json                      │ Tracks last-released versions (cc/node/deno)  │
│ templates/claude-{node,deno}.cmd   │ Windows wrapper scripts                       │
│ templates/claude-{node,deno}.sh    │ Linux wrapper scripts                         │
│ debian/control.{node,deno}.template│ DEBIAN/control templates for .deb packaging   │
...
Runner mapping: `windows-latest` builds win-x64 and win-arm64 (cross-build), `ubuntu-latest` builds linux-x64, `ubuntu-24.04-arm` builds linux-arm64.
...
│ Job            │ What it does                                            │
├────────────────┼─────────────────────────────────────────────────────────┤
│ check-versions │ Fetches latest cc/node/deno versions, compares with     │
│                │ versions.json, sets needs_build flags                   │
...
Release jobs pull before committing to avoid merge conflicts when both run concurrently.
...
- Deno .deb uses `Conflicts: claude-code` so only one variant installs at a time.
